...
---
The "mapSOOSWG" package was written to semi-automate the creation of maps for SOOS reporting purposes. The package was created with some flexibility. This documentation details the functions, provide examples on how to use them and illustrates where map style can be altered slightly. 
...
If the below plot was produced, the package is working. Don't worry if it appears a littl blurry, thats normal. Time to get started!
...
|Input data frame | R environment name | Example filename | Original File                                          |
|----------|--------------------|---------------------------------------------------------|
...
# read the file
data = read.csv(lookupfile, header = T, stringsAsFactors = F,strip.white = T)
...
# read the file
data = read.csv(inst_LT, header = T, stringsAsFactors = F,strip.white = T)
colnames(data) = gsub("ï..","",colnames(data)) # this is a bug that can occur from excel (yay excel)
```
...
# read the file
data = read.csv(cat_table, header = T, stringsAsFactors = F,strip.white = T)
colnames(data) = gsub("ï..","",colnames(data)) # this is a bug that can occur from excel (yay excel)
```
...
```{r, eval = F, echo = T}
geocode_maps(SOOS_WG_data = example_WG_data, outdir = "./output",institution_names_data = example_country_data, country_names_data = example_country_data)
```
To create maps with hilighted participating countries and geocoded instititutions:
```{r, eval = F, echo = T}
geocode_maps(SOOS_WG_data = example_WG_data, outdir = "./output",institution_names_data = example_country_data, add_countries = F)
```
...
```{r, eval = F, echo = T}
category_maps(outdir = "./output",category_data = example_category_data, country_names_data = example_country_data, plot_type = "geocode", institution_names_data = example_institution_data, plot_legend = F)
```
Changing category names:
```{r, eval = F, echo = T}
category_maps(outdir = "./output",category_data = example_category_data, country_names_data = example_country_data, plot_type = "geocode", institution_names_data = example_institution_data, category_names = c("A","B","C","D"))
```
Changing legend title:
```{r, eval = F, echo = T}
category_maps(outdir = "./output",category_data = example_category_data, country_names_data = example_country_data, plot_type = "geocode", institution_names_data = example_institution_data, category_names = c("A","B","C","D"), category_legend = "Grade")
```
...
Note that the marker cannot have an outline with this option.
```{r, eval = F, echo = T}
WG_maps(example_WG_data, outdir = "./output", example_country_data)
```
